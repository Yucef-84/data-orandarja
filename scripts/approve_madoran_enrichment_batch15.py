"""Freeze the HeadGPT-approved MADOran Batch 15 review state."""
...
from scripts.build_madoran_enrichment_batch15 import BATCH_OUT, TARGET_END, TARGET_START
...
REVIEW_ID = "MADORAN-ENRICH-015-REVIEW-01"
REVIEWED_COMMIT = "78016b23e628499674f1880fbfa2d775fdd6f374"
...
REVIEW_OUT = ROOT / "data/master/qa/madoran_enrichment_batch15_review.json"
...
def apply() -> dict[str, object]:
    ...
    if event_check["result"] != "PASS" or event_check["events"] != 13063 or trace["result"] != "PASS" or trace["populated_fields"] != 11332:
    ...
    if len(target) != 64 or len(flagged) != 61 or len(drafts) != 3:
        raise RuntimeError("unexpected_batch15_preapproval_state")
    ...
    expected = {"qa_passed": 320, "flagged": 640, "draft": 0, "not_started": 396}
    if counts != expected:
    ...
    review = {
        ...
        "qa_passed_rows": 3, "flagged_rows": flagged, "not_started_rows": 396,
        "batch15_state_before": {"draft": 3, "flagged": 61},
        "batch15_state_after": {"qa_passed": 3, "flagged": 61},
        ...
        "arabic_modified": 0, "correction_events_total": 847, "provenance_events_before": 12216,
        "provenance_events_after": 13063, "provenance_modified": 0, "outside_target_mutations": 0,
        "batch_artifact_sync": "PASS", "reviewed_batch_rows": 64, "reviewed_at": reviewed_at,
        "review_summary": (
            "HeadGPT independently rechecked all 64 rows against canonical Arabic at the final correction04 commit. "
        ...
        "next_batch": {"batch_id": "MADORAN-ENRICH-016", "sentno_start": 961, "sentno_end": 1024, "row_count": 64},
    }

    status = json.loads(STATUS_OUT.read_text(encoding="utf-8"))
    status["decision_id"] = "HEAD-MADORAN-2026-08-12-15"
    status["updated_from_commit"] = REVIEWED_COMMIT
    status["enrichment_batch_id"] = BATCH_ID
    status["enrichment_review_id"] = REVIEW_ID
    status["counts"] = {
        "enrichment_completed_rows": 320, "enrichment_draft_rows": 0,
        "enrichment_flagged_rows": 640, "enrichment_not_started_rows": 396,
        "processing_flags_populated_rows": 772,
    }
