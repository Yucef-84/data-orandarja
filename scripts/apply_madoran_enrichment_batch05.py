"""Apply MADOran Batch 05 with source-only provenance and isolation gates."""
...
from scripts.build_madoran_enrichment_batch05 import (
    BATCH_FIELDS,
    BATCH_ID,
    BATCH_OUT,
    BASE_COMMIT,
...
def validate_applied_state(before_rows: list[dict[str, str]], after_rows: list[dict[str, str]], batch_rows: list[dict[str, str]]) -> dict[str, object]:
    failures: list[str] = []
...
def apply() -> dict[str, object]:
    ...
    if check_layer_contract()["result"] != "PASS":
        raise RuntimeError("layer_contract_failed")
    check_manifest()
    source_rows = read_tsv(SOURCE_OUT)
    before_rows = read_tsv(ENRICHMENT_OUT)
    batch_rows = read_tsv(BATCH_OUT)
    if list(before_rows[0]) != ENRICHMENT_FIELDS or list(batch_rows[0]) != BATCH_FIELDS:
        raise RuntimeError("header_mismatch")
    ...
    if existing_check["result"] != "PASS" or existing_check["events"] != EXPECTED_EXISTING_EVENTS:
    ...
    for row in after_rows:
        if TARGET_START <= int(row["sentno"]) <= TARGET_END:
            if row["enrichment_state"] != "not_started":
                raise RuntimeError(f"target_row_already_processed:{row['sentno']}")
            batch = target_by_sentno[row["sentno"]]
            for field in (*EMPTY_FIELDS, "processing_flags"):
    ...
    new_events = [event(row["source_uid"], field, row[field], generated_at) for row in batch_rows for field in EMPTY_FIELDS]
    new_events.extend(event(row["source_uid"], "processing_flags", row["processing_flags"], generated_at) for row in batch_rows if row["processing_flags"])
    ...
    for evidence in ("data/master/qa/madoran_enrichment_batch05_generation_qa.json", "data/master/qa/madoran_enrichment_batch05_qa.json"):
    ...
    qa = {
        ...
        "populated_linguistic_fields": sum(bool(row[field]) for row in after_rows for field in EMPTY_FIELDS),
        ...
        "expected_total_provenance_events": expected_total,
        "missing_provenance_events": 0,
        "hash_mismatch": 0,
        "prefix_preserved": combined_event_text.startswith(existing_event_text),
        "source_gate": "PASS",
        "layer_contract": "PASS",
        ...
        "generated_at": generated_at,
        "outputs": {"batch": BATCH_OUT.relative_to(ROOT).as_posix(), "manifest": MANIFEST_OUT.relative_to(ROOT).as_posix(), "enrichment": ENRICHMENT_OUT.relative_to(ROOT).as_posix(), "provenance": EVENTS_OUT.relative_to(ROOT).as_posix()},
    }
