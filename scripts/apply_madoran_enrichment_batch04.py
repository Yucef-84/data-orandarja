"""Apply MADOran Batch 04 with source-only provenance and isolation gates."""
...
from scripts.build_madoran_enrichment_batch04 import (
    BATCH_FIELDS,
    BATCH_ID,
    BATCH_OUT,
    BASE_COMMIT,
...
def validate_batch_rows(
    ...
    return {
        ...
        "processing_flags_populated_rows": sum(
            bool(row.get("processing_flags")) for row in batch_rows
        ),
...
def apply() -> dict[str, object]:
    ...
    if (
        existing_check["result"] != "PASS"
        or existing_check["events"] != EXPECTED_EXISTING_EVENTS
    ):
    ...
    status["counts"] = {
        "enrichment_completed_rows": sum(
            row["enrichment_state"] in {"qa_passed", "reviewed"} for row in after_rows
        ),
        "enrichment_draft_rows": sum(
            row["enrichment_state"] == "draft" for row in after_rows
        ),
        "enrichment_flagged_rows": sum(
            row["enrichment_state"] == "flagged" for row in after_rows
        ),
        "enrichment_not_started_rows": sum(
            row["enrichment_state"] == "not_started" for row in after_rows
        ),
        "processing_flags_populated_rows": sum(
            bool(row["processing_flags"]) for row in after_rows
        ),
    }
    evidence_files = list(status.get("evidence_files", []))
    for evidence in (
        "data/master/qa/madoran_enrichment_batch04_generation_qa.json",
        "data/master/qa/madoran_enrichment_batch04_qa.json",
    ):
    ...
    qa = {
        ...
        "populated_linguistic_fields": sum(
