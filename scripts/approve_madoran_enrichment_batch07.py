"""Freeze the HeadGPT-approved MADOran Batch 07 review state."""
...
from scripts.build_madoran_enrichment_batch07 import BATCH_OUT, TARGET_END, TARGET_START
...
REVIEW_ID = "MADORAN-ENRICH-007-REVIEW-01"
REVIEWED_COMMIT = "f6541136d6067255c8f9643653a263d73fae4b3c"
...
def apply() -> dict:
    ...
    if (
        ...
        or event_check["events"] != 6228
        or trace["result"] != "PASS"
        or trace["populated_fields"] != 5225
    ):
    ...
    if len(target) != 64 or len(flagged) != 41 or len(drafts) != 23:
        raise RuntimeError("unexpected_batch07_preapproval_state")
    ...
    expected = {"qa_passed": 280, "flagged": 168, "draft": 0, "not_started": 908}
    if counts != expected:
    ...
    review = {
        ...
        "not_started_rows": 908,
        "batch07_state_before": {"draft": 23, "flagged": 41},
        "batch07_state_after": {"qa_passed": 23, "flagged": 41},
        ...
        "arabic_modified": 0,
        "correction_events_total": 65,
        "provenance_events_before": 6228,
        "provenance_events_after": 6228,
        ...
        "review_summary": (
            "HeadGPT independently rechecked all 64 rows against canonical Arabic. The second correction set "
            "resolved the remaining issues in Sentno 391, 393, 399, 404, 410, 418, 422, 426, and 441; no new "
            "P0/P1 or regressions were found."
        ),
        "next_batch": {"batch_id": "MADORAN-ENRICH-008", "sentno_start": 449, "sentno_end": 512, "row_count": 64},
    }

    status = json.loads(STATUS_OUT.read_text(encoding="utf-8"))
    status["decision_id"] = "HEAD-MADORAN-2026-08-11-04"
    status["updated_from_commit"] = REVIEWED_COMMIT
    status["enrichment_batch_id"] = BATCH_ID
    status["enrichment_review_id"] = REVIEW_ID
    status["counts"] = {
        "enrichment_completed_rows": 280,
        "enrichment_draft_rows": 0,
        "enrichment_flagged_rows": 168,
        "enrichment_not_started_rows": 908,
    ...
    qa.update(
        {
            ...
            "reviewed_commit": REVIEWED_COMMIT,
            "reviewed_at": reviewed_at,
            "approved_draft_rows": 23,
            "approved_flagged_rows": 41,
        }
    )
