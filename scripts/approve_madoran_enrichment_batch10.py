"""Freeze the HeadGPT-approved MADOran Batch 10 review state."""
...
from scripts.build_madoran_enrichment_batch10 import BATCH_OUT, TARGET_END, TARGET_START
...
REVIEW_ID = "MADORAN-ENRICH-010-REVIEW-01"
REVIEWED_COMMIT = "1f2a0697418b35affed6cf7383d73691b4ea1d72"
...
def apply() -> dict[str, object]:
    ...
    if event_check["result"] != "PASS" or event_check["events"] != 8655 or trace["result"] != "PASS" or trace["populated_fields"] != 7505:
    ...
    if len(target) != 64 or len(flagged) != 61 or len(drafts) != 3:
        raise RuntimeError("unexpected_batch10_preapproval_state")
    ...
    expected = {"qa_passed": 305, "flagged": 335, "draft": 0, "not_started": 716}
    if counts != expected:
    ...
    review = {
        ...
        "qa_passed_rows": 3, "flagged_rows": flagged, "not_started_rows": 716,
        "batch10_state_before": {"draft": 3, "flagged": 61},
        "batch10_state_after": {"qa_passed": 3, "flagged": 61},
        ...
        "arabic_modified": 0, "correction_events_total": 835, "provenance_events_before": 7820,
        "provenance_events_after": 8655, "provenance_modified": 0, "outside_target_mutations": 0,
        ...
        "review_summary": (
            ...
            "Correction rounds resolved the identified source-ambiguity, historical, code-switching, speaker, and "
            "toilet-food joke issues; no P0/P1 or regressions remained."
        ),
        "next_batch": {"batch_id": "MADORAN-ENRICH-011", "sentno_start": 641, "sentno_end": 704, "row_count": 64},
    }

    status = json.loads(STATUS_OUT.read_text(encoding="utf-8"))
    status["decision_id"] = "HEAD-MADORAN-2026-08-11-07"
    status["updated_from_commit"] = REVIEWED_COMMIT
    status["enrichment_batch_id"] = BATCH_ID
    status["enrichment_review_id"] = REVIEW_ID
    status["counts"] = {
        "enrichment_completed_rows": 305, "enrichment_draft_rows": 0,
        "enrichment_flagged_rows": 335, "enrichment_not_started_rows": 716,
        "processing_flags_populated_rows": 465,
    }
