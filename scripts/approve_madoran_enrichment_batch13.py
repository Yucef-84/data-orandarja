"""Freeze the HeadGPT-approved MADOran Batch 13 review state."""
...
from scripts.build_madoran_enrichment_batch13 import BATCH_OUT, TARGET_END, TARGET_START
...
REVIEW_ID = "MADORAN-ENRICH-013-REVIEW-01"
REVIEWED_COMMIT = "c920256326e9a77557679b78934b9a507c4573e8"
...
REVIEW_OUT = ROOT / "data/master/qa/madoran_enrichment_batch13_review.json"
...
def apply() -> dict[str, object]:
    ...
    if event_check["result"] != "PASS" or event_check["events"] != 11312 or trace["result"] != "PASS" or trace["populated_fields"] != 9801:
    ...
    if len(target) != 64 or len(flagged) != 61 or len(drafts) != 3:
        raise RuntimeError("unexpected_batch13_preapproval_state")
    ...
    expected = {"qa_passed": 314, "flagged": 518, "draft": 0, "not_started": 524}
    if counts != expected:
    ...
    review = {
        ...
        "qa_passed_rows": 3, "flagged_rows": flagged, "not_started_rows": 524,
        "batch13_state_before": {"draft": 3, "flagged": 61},
        "batch13_state_after": {"qa_passed": 3, "flagged": 61},
        ...
        "arabic_modified": 0, "correction_events_total": 897, "provenance_events_before": 10415,
        "provenance_events_after": 11312, "provenance_modified": 0, "outside_target_mutations": 0,
        ...
        "review_summary": (
            ...
            "Correction rounds preserved source-only speaker, actor, turn-boundary, time-axis, amount, legal, "
            "kinship, Oran-context, and code-switch semantics; no P0/P1 or regressions remained."
        ),
        "next_batch": {"batch_id": "MADORAN-ENRICH-014", "sentno_start": 833, "sentno_end": 896, "row_count": 64},
    }

    status = json.loads(STATUS_OUT.read_text(encoding="utf-8"))
    status["decision_id"] = "HEAD-MADORAN-2026-08-11-10"
    status["updated_from_commit"] = REVIEWED_COMMIT
    status["enrichment_batch_id"] = BATCH_ID
    status["enrichment_review_id"] = REVIEW_ID
    status["counts"] = {
        "enrichment_completed_rows": 314, "enrichment_draft_rows": 0,
        "enrichment_flagged_rows": 518, "enrichment_not_started_rows": 524,
        "processing_flags_populated_rows": 649,
    }
