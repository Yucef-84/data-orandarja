"""Freeze the HeadGPT-approved MADOran Batch 16 review state."""
...
from scripts.build_madoran_enrichment_batch16 import BATCH_OUT, TARGET_END, TARGET_START
...
REVIEW_ID = "MADORAN-ENRICH-016-REVIEW-01"
REVIEWED_COMMIT = "992d0940844cbc0f0de37af9d8da7eed6b0d0b70"
...
REVIEW_OUT = ROOT / "data/master/qa/madoran_enrichment_batch16_review.json"
...
def apply() -> dict[str, object]:
    ...
    if event_check["result"] != "PASS" or event_check["events"] != 13943 or trace["result"] != "PASS" or trace["populated_fields"] != 12097:
    ...
    if len(target) != 64 or len(flagged) != 61 or len(drafts) != 3:
        raise RuntimeError("unexpected_batch16_preapproval_state")
    ...
    expected = {"qa_passed": 323, "flagged": 701, "draft": 0, "not_started": 332}
    if counts != expected:
    ...
    review = {
        ...
        "qa_passed_rows": 3, "flagged_rows": flagged, "not_started_rows": 332,
        "batch16_state_before": {"draft": 3, "flagged": 61},
        "batch16_state_after": {"qa_passed": 3, "flagged": 61},
        ...
        "arabic_modified": 0, "correction_events_total": 880, "provenance_events_before": 13063,
        "provenance_events_after": 13943, "provenance_modified": 0, "outside_target_mutations": 0,
        "batch_artifact_sync": "PASS", "reviewed_batch_rows": 64, "reviewed_at": reviewed_at,
        "review_summary": (
            "HeadGPT independently rechecked all 64 rows against canonical Arabic at the final correction06 commit. "
            "Correction rounds preserved source-only speaker, actor, turn-boundary, time-axis, amount, kinship, "
            "code-switch, opaque-form, direct-register, and speech-act semantics; no P0/P1 or regressions remained."
        ),
        "next_batch": {"batch_id": "MADORAN-ENRICH-017", "sentno_start": 1025, "sentno_end": 1088, "row_count": 64},
    }

    status = json.loads(STATUS_OUT.read_text(encoding="utf-8"))
    status["decision_id"] = "HEAD-MADORAN-2026-08-12-16"
    status["updated_from_commit"] = REVIEWED_COMMIT
    status["enrichment_batch_id"] = BATCH_ID
    status["enrichment_review_id"] = REVIEW_ID
    status["counts"] = {
        "enrichment_completed_rows": 323, "enrichment_draft_rows": 0,
        "enrichment_flagged_rows": 701, "enrichment_not_started_rows": 332,
        "processing_flags_populated_rows": 833,
    }
