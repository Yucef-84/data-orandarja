"""Freeze the HeadGPT-approved MADOran Batch 04 review state."""
...
from scripts.apply_madoran_enrichment_batch04 import BATCH_OUT, validate_batch_rows
from scripts.build_madoran_enrichment_batch04 import TARGET_END, TARGET_START
...
REVIEW_ID = "MADORAN-ENRICH-004-REVIEW-01"
REVIEWED_COMMIT = "0df2021"
TARGET_FLAGGED = [
...
def apply() -> dict[str, object]:
    ...
    if (
        ...
        or event_check["events"] != 3819
        or trace["result"] != "PASS"
        or trace["populated_fields"] != 2970
    ):
    ...
    if len(target_rows) != 64 or flagged_before != TARGET_FLAGGED or len(draft_before) != 41:
        raise RuntimeError("unexpected_batch04_preapproval_state")
    ...
    for before, after in zip(before_rows, after_rows):
        ...
        if TARGET_START <= int(sentno) <= TARGET_END:
            if after["enrichment_state"] == "draft":
                ...
                raise RuntimeError(f"unexpected_batch04_state:{sentno}")
        elif after != before:
    ...
    status["decision_id"] = "HEAD-MADORAN-2026-08-11-02"
    ...
    review = {
        ...
        "not_started_rows": 1100,
        "batch04_state_before": {"draft": 41, "flagged": 23},
        "batch04_state_after": {"qa_passed": 41, "flagged": 23},
        ...
        "arabic_modified": 0,
        "provenance_events_before": 3819,
        "provenance_events_after": 3819,
