"""Freeze the HeadGPT-approved MADOran Batch 11 review state."""
...
from scripts.build_madoran_enrichment_batch11 import BATCH_OUT, TARGET_END, TARGET_START
...
BATCH_ID = "MADORAN-ENRICH-011"
REVIEW_ID = "MADORAN-ENRICH-011-REVIEW-01"
REVIEWED_COMMIT = "208787d7982cfe08e1cc0204fc91ad0dd93c632d"
...
def apply() -> dict[str, object]:
    ...
    if event_check["result"] != "PASS" or event_check["events"] != 9479 or trace["result"] != "PASS" or trace["populated_fields"] != 8270:
    ...
    if len(target) != 64 or len(flagged) != 61 or len(drafts) != 3:
        raise RuntimeError("unexpected_batch11_preapproval_state")
    ...
    expected = {"qa_passed": 308, "flagged": 396, "draft": 0, "not_started": 652}
    if counts != expected:
    ...
    review = {
        ...
        "qa_passed_rows": 3, "flagged_rows": flagged, "not_started_rows": 652,
        "batch11_state_before": {"draft": 3, "flagged": 61},
        "batch11_state_after": {"qa_passed": 3, "flagged": 61},
        ...
        "arabic_modified": 0, "correction_events_total": 824, "provenance_events_before": 8655,
        "provenance_events_after": 9479, "provenance_modified": 0, "outside_target_mutations": 0,
        ...
        "review_summary": (
            ...
            "Correction rounds preserved source-only food, numeric, code-switch, speaker, actor, kinship, "
            "wordplay, and topic semantics; no P0/P1 or regressions remained."
        ),
        "next_batch": {"batch_id": "MADORAN-ENRICH-012", "sentno_start": 705, "sentno_end": 768, "row_count": 64},
    }

    status = json.loads(STATUS_OUT.read_text(encoding="utf-8"))
    status["decision_id"] = "HEAD-MADORAN-2026-08-11-08"
    status["updated_from_commit"] = REVIEWED_COMMIT
    status["enrichment_batch_id"] = BATCH_ID
    status["enrichment_review_id"] = REVIEW_ID
    status["counts"] = {
        "enrichment_completed_rows": 308, "enrichment_draft_rows": 0,
        "enrichment_flagged_rows": 396, "enrichment_not_started_rows": 652,
        "processing_flags_populated_rows": 526,
    }
