"""Apply MADORAN-ENRICH-002 with source-only provenance and state gates."""
...
from scripts.build_madoran_enrichment_batch02 import (
    BATCH_FIELDS,
    BATCH_ID,
    BATCH_OUT,
    BASE_COMMIT,
...
def expected_state(flags: str) -> str:
    return "flagged" if any(
        value in flags.split("|") for value in ("source_ambiguity", "source_corruption")
    ) else "draft"
...
def validate_applied_state(before_rows: list[dict[str, str]], after_rows: list[dict[str, str]], batch_rows: list[dict[str, str]]) -> dict[str, object]:
    ...
    for before, after in zip(before_rows, after_rows):
        ...
        if TARGET_START <= int(sentno) <= TARGET_END:
            ...
            if after.get("enrichment_state") != expected_state(batch["processing_flags"]):
    ...
    return {
        ...
        "flagged_rows": sum(1 for row in batch_rows if expected_state(row["processing_flags"]) == "flagged"),
        "draft_rows": sum(1 for row in batch_rows if expected_state(row["processing_flags"]) == "draft"),
    }
...
def apply() -> dict[str, object]:
    ...
    for row in after_rows:
        ...
        if TARGET_START <= int(sentno) <= TARGET_END:
            ...
            row["enrichment_state"] = expected_state(batch["processing_flags"])
    ...
    qa = {
        ...
        "processing_flags_populated_rows": batch_check["processing_flags_populated_rows"],
        "state_transition": "not_started_to_draft_or_flagged",
        "populated_linguistic_fields": len(batch_rows) * len(EMPTY_FIELDS),
