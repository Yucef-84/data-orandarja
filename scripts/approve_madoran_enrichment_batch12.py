"""Freeze the HeadGPT-approved MADOran Batch 12 review state."""
...
from scripts.build_madoran_enrichment_batch12 import BATCH_OUT, TARGET_END, TARGET_START
...
BATCH_ID = "MADORAN-ENRICH-012"
REVIEW_ID = "MADORAN-ENRICH-012-REVIEW-01"
REVIEWED_COMMIT = "1eeddf78b6ff9990144a685bb1d730a6d2d8e021"
...
def apply() -> dict[str, object]:
    ...
    if event_check["result"] != "PASS" or event_check["events"] != 10415 or trace["result"] != "PASS" or trace["populated_fields"] != 9035:
    ...
    if len(target) != 64 or len(flagged) != 61 or len(drafts) != 3:
        raise RuntimeError("unexpected_batch12_preapproval_state")
    ...
    expected = {"qa_passed": 311, "flagged": 457, "draft": 0, "not_started": 588}
    if counts != expected:
    ...
    review = {
        ...
        "qa_passed_rows": 3, "flagged_rows": flagged, "not_started_rows": 588,
        "batch12_state_before": {"draft": 3, "flagged": 61},
        "batch12_state_after": {"qa_passed": 3, "flagged": 61},
        ...
        "arabic_modified": 0, "correction_events_total": 936, "provenance_events_before": 9479,
        "provenance_events_after": 10415, "provenance_modified": 0, "outside_target_mutations": 0,
        ...
        "review_summary": (
            ...
            "Correction rounds preserved source-only speaker, actor, time-axis, proverb, culture-word, food, "
            "Oran-context, and code-switch semantics; no P0/P1 or regressions remained."
        ),
        "next_batch": {"batch_id": "MADORAN-ENRICH-013", "sentno_start": 769, "sentno_end": 832, "row_count": 64},
    }

    status = json.loads(STATUS_OUT.read_text(encoding="utf-8"))
    status["decision_id"] = "HEAD-MADORAN-2026-08-11-09"
    status["updated_from_commit"] = REVIEWED_COMMIT
    status["enrichment_batch_id"] = BATCH_ID
    status["enrichment_review_id"] = REVIEW_ID
    status["counts"] = {
        "enrichment_completed_rows": 311, "enrichment_draft_rows": 0,
        "enrichment_flagged_rows": 457, "enrichment_not_started_rows": 588,
        "processing_flags_populated_rows": 587,
    }
