"""Freeze the HeadGPT-approved MADOran Batch 17 review state."""
...
from scripts.build_madoran_enrichment_batch17 import BATCH_OUT, TARGET_END, TARGET_START
...
REVIEW_ID = "MADORAN-ENRICH-017-REVIEW-01"
REVIEWED_COMMIT = "e0de4cb"
...
def apply() -> dict[str, object]:
    ...
    if not head.startswith(REVIEWED_COMMIT) or source_gate()["result"] != "PASS":
        raise RuntimeError(f"reviewed_commit_or_source_mismatch:{head}")
    ...
    if event_check["result"] != "PASS" or event_check["events"] != 14973 or trace["result"] != "PASS" or trace["populated_fields"] != 12863:
    ...
    if len(target) != 64 or len(flagged) != 57 or len(drafts) != 7:
        raise RuntimeError("unexpected_batch17_preapproval_state")
    ...
    expected = {"qa_passed": 330, "flagged": 758, "draft": 0, "not_started": 268}
    if counts != expected:
        raise RuntimeError(json.dumps(counts))

    reviewed_at = now()
    review = {
        "review_id": REVIEW_ID, "batch_id": BATCH_ID, "reviewed_commit": head,
        "reviewer": "HeadGPT", "headgpt_result": "PASS", "p0": "NONE", "p1": "NONE", "p2": "NONE",
        "structure": "PASS", "provenance": "PASS", "isolation": "PASS", "next_batch_allowed": True,
        "qa_passed_rows": 7, "flagged_rows": flagged, "not_started_rows": 268,
        "batch17_state_before": {"draft": 7, "flagged": 57},
        "batch17_state_after": {"qa_passed": 7, "flagged": 57},
        ...
        "arabic_modified": 0, "correction_events_total": 1030, "provenance_events_before": 13943,
        "provenance_events_after": 14973, "provenance_modified": 0, "outside_target_mutations": 0,
        "batch_artifact_sync": "PASS", "reviewed_batch_rows": 64, "reviewed_at": reviewed_at,
        "review_summary": (
            "HeadGPT independently rechecked all 64 rows against the canonical source at the final "
            "correction-05 and QA-alignment commit. Meaning, structure, provenance, and isolation passed; "
            "no P0/P1/P2 issues remained and the next batch was allowed."
        ),
        "next_batch": {"batch_id": "MADORAN-ENRICH-018", "sentno_start": 1089, "sentno_end": 1152, "row_count": 64},
    }

    status = json.loads(STATUS_OUT.read_text(encoding="utf-8"))
    status["decision_id"] = "HEAD-MADORAN-2026-08-12-17"
    status["updated_from_commit"] = head
    status["enrichment_batch_id"] = BATCH_ID
    status["enrichment_review_id"] = REVIEW_ID
    status["counts"] = {
        "enrichment_completed_rows": 330, "enrichment_draft_rows": 0,
        "enrichment_flagged_rows": 758, "enrichment_not_started_rows": 268,
        "processing_flags_populated_rows": 895,
    }
    ...
    qa.update({
        ...
        "reviewed_commit": head, "reviewed_at": reviewed_at,
        "approved_draft_rows": 7, "approved_flagged_rows": 57,
    })
