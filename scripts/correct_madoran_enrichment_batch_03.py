"""Apply the third HeadGPT-directed correction pass for MADOran Batch 01."""
...
CORRECTION_ID = "MADORAN-ENRICH-001-CORRECTION-03"
CORRECTION_PROMPT_VERSION = "madoran-source-enrichment-v1-correction-3"
...
def apply() -> dict[str, object]:
    ...
    if existing_event_check["result"] != "PASS" or existing_event_check["events"] != 1429:
        ...
        if TARGET_START <= int(sentno) <= TARGET_END:
            ...
            for field in EMPTY_FIELDS:
                ...
                if latest["value_hash"] != current_hash:
                    changed_fields.append((after["source_uid"], field, corrected[field]))
                after[field] = corrected[field]
            if sentno == "17":
                after["enrichment_state"] = "flagged"
            if sentno == "63" and after["enrichment_state"] != "flagged":
                raise RuntimeError("sentno_63_state_regressed")
        elif before != after:
    ...
    correction_events = [_event(uid, field, value, generated_at) for uid, field, value in changed_fields]
    ...
    with EVENTS_OUT.open("a", encoding="utf-8", newline="") as handle:
        handle.write(correction_text)
