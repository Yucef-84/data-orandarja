"""Freeze the HeadGPT-approved MADOran Batch 06 review state."""
...
from scripts.apply_madoran_enrichment_batch06 import BATCH_OUT, TARGET_END, TARGET_START, validate_batch_rows
...
REVIEW_ID = "MADORAN-ENRICH-006-REVIEW-01"
REVIEWED_COMMIT = "5d434f6704906b551c5475dd8a1f3988823e266c"
...
def apply() -> dict:
    ...
    if (
        ...
        or event_check["events"] != 5413
        or trace["result"] != "PASS"
        or trace["populated_fields"] != 4473
    ):
    ...
    if len(target) != 64 or len(flagged) != 40 or len(drafts) != 24:
        raise RuntimeError("unexpected_batch06_preapproval_state")
    ...
    expected = {"qa_passed": 257, "flagged": 127, "draft": 0, "not_started": 972}
    if counts != expected:
    ...
    review = {
        ...
        "not_started_rows": 972,
        "batch06_state_before": {"draft": 24, "flagged": 40},
        "batch06_state_after": {"qa_passed": 24, "flagged": 40},
        ...
        "next_batch": {"batch_id": "MADORAN-ENRICH-007", "sentno_start": 385, "sentno_end": 448, "row_count": 64},
    }
    ...
    qa.update(
        {
            ...
            "reviewed_commit": REVIEWED_COMMIT,
            "reviewed_at": reviewed_at,
            "approved_draft_rows": 24,
            "approved_flagged_rows": 40,
        }
    )
