...
class MadoranEnrichmentBatch05Tests(unittest.TestCase):
    ...
    def test_batch05_state_distribution_and_ascii(self):
        ...
        self.assertEqual(sum(row["enrichment_state"] == "draft" for row in target), 36)
        self.assertEqual(sum(row["enrichment_state"] == "flagged" for row in target), 28)
        self.assertTrue(all(row["latin"].isascii() for row in target))
        self.assertEqual(sum(bool(row["processing_flags"]) for row in target), 49)
    ...
    def test_batch05_generation_and_application_qa_pass(self):
        ...
        self.assertEqual(application["new_provenance_events"], 704 + 49)
        self.assertEqual(application["total_provenance_events"], 4572)
        self.assertEqual(application["expected_total_provenance_events"], 4572)
        self.assertEqual(application["draft_rows"], 36)
        self.assertEqual(application["flagged_rows"], 28)
    ...
    def test_batch05_ambiguous_rows_are_explicitly_flagged(self):
        ...
        for sentno in (258, 259, 266, 267, 268, 270, 271, 273, 274, 276, 279, 282, 283, 284, 286, 291, 304, 306, 307, 309, 311, 312, 313, 314, 315, 318, 319):
