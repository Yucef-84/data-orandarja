...
class MadoranEnrichmentBatchTests(unittest.TestCase):
    ...
    def test_applied_state_is_exactly_targeted(self):
        ...
        outside = [row for row in self.enrichment_rows if int(row["sentno"]) > 192]
        ...
        self.assertEqual(sum(row["enrichment_state"] == "draft" for row in batch03), 51)
        self.assertEqual(sum(row["enrichment_state"] == "flagged" for row in batch03), 13)
    ...
    def test_provenance_is_field_level_and_hashed(self):
        ...
        self.assertEqual(trace["populated_fields"], 3 * 64 * len(EMPTY_FIELDS) + 92)
