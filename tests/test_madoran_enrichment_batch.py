...
class MadoranEnrichmentBatchTests(unittest.TestCase):
    ...
    def test_applied_state_is_exactly_targeted(self):
        ...
        outside = [row for row in self.enrichment_rows if int(row["sentno"]) > 256]
        ...
        self.assertEqual(sum(row["enrichment_state"] == "draft" for row in batch04), 43)
        self.assertEqual(sum(row["enrichment_state"] == "flagged" for row in batch04), 21)
    ...
    def test_provenance_is_field_level_and_hashed(self):
        ...
        self.assertEqual(trace["populated_fields"], 4 * 64 * len(EMPTY_FIELDS) + 154)
