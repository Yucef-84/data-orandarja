...
class ContextualBatch02Tests(unittest.TestCase):
    ...
    def test_batch02_is_pre_review_only(self):
        current = rows(BATCH02)
        self.assertTrue(all(row["review_status"] in {"source_verified", "gpt_reviewed", "hold"} for row in current))
    ...
    def test_batch02_hold_policy(self):
        ...
        self.assertEqual(sum(row["review_status"] == "gpt_reviewed" for row in current), 87)
