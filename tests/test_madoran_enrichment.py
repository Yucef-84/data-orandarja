...
class MadoranEnrichmentTests(unittest.TestCase):
    ...
    def test_non_empty_initial_field_is_rejected(self):
        mutated = [dict(row) for row in self.enrichment_rows]
        mutated[0]["enrichment_state"] = "not_started"
    ...
    def test_headgpt_approved_state_is_frozen(self):
        ...
        self.assertEqual(
            sum(row["enrichment_state"] == "qa_passed" for row in self.enrichment_rows[128:192]),
            50,
        )
        self.assertEqual(
            sum(row["enrichment_state"] == "flagged" for row in self.enrichment_rows[128:192]),
            14,
        )
        self.assertEqual(
        ...
        self.assertTrue(
            all(
                row["enrichment_state"] in {"draft", "flagged", "qa_passed"}
        ...
        batch20 = [row for row in self.enrichment_rows if 1217 <= int(row["sentno"]) <= 1280]
        ...
        self.assertTrue(
            all(row["enrichment_state"] == "not_started" for row in self.enrichment_rows if int(row["sentno"]) > 1344)
        )
    ...
    def test_processing_flags_are_source_metadata_not_workflow_state(self):
        ...
        self.assertEqual(sum(bool(row["processing_flags"]) for row in self.enrichment_rows), 1122)
        self.assertEqual(self.enrichment_rows[16]["enrichment_state"], "flagged")
        self.assertEqual(self.enrichment_rows[62]["enrichment_state"], "flagged")
    ...
    def test_provenance_event_log_contains_batch_events(self):
        ...
        self.assertEqual(report["result"], "PASS")
        self.assertEqual(report["events"], 19038)
    ...
    def test_processing_flag_provenance_hashes_are_current(self):
        ...
        self.assertEqual(report["populated_fields"], 15906)
