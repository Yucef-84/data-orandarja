...
from scripts.build_madoran_enrichment_batch12 import (
...
CORRECTION03_QA_OUT = ROOT / "data/master/qa/madoran_enrichment_batch12_correction03_qa.json"
CORRECTION04_QA_OUT = ROOT / "data/master/qa/madoran_enrichment_batch12_correction04_qa.json"
CORRECTION05_QA_OUT = ROOT / "data/master/qa/madoran_enrichment_batch12_correction05_qa.json"
CORRECTION06_QA_OUT = ROOT / "data/master/qa/madoran_enrichment_batch12_correction06_qa.json"
...
class MadoranEnrichmentBatch12Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_batch12_header_and_range(self):
    ...
    def test_batch12_source_only_validation_passes(self):
    ...
    def test_batch12_manifest_contract(self):
    ...
    def test_batch12_provenance_and_latest_hash_gate_pass(self):
        ...
        self.assertEqual(event_check["events"], 10415)
        trace = check_enrichment_provenance(self.enrichment_rows, self.event_text)
        self.assertEqual(trace["result"], "PASS", trace)
        self.assertEqual(trace["populated_fields"], 9035)

    def test_batch12_generation_and_application_qa_pass(self):
        ...
        self.assertEqual(application["provenance_events_before"], 9479)
        self.assertEqual(application["new_provenance_events"], 936)
        self.assertEqual(application["total_provenance_events"], 10415)
    ...
    def test_batch12_correction01_qa_pass(self):
        ...
        self.assertEqual(correction["correction_id"], "MADORAN-ENRICH-012-CORRECTION-01")
        self.assertEqual(correction["changed_fields"], 125)
        self.assertEqual(correction["provenance_events_before"], 10245)
        self.assertEqual(correction["provenance_events_after"], 10370)
        self.assertEqual(correction["validator"], "PASS")
        self.assertEqual(correction["batch_artifact_sync"], "PASS")

    def test_batch12_correction02_qa_pass(self):
        correction = json.loads(CORRECTION02_QA_OUT.read_text(encoding="utf-8"))
        self.assertEqual(correction["result"], "PASS")
        self.assertEqual(correction["correction_id"], "MADORAN-ENRICH-012-CORRECTION-02")
        self.assertEqual(correction["corrected_rows"], ["731", "736", "746"])
        self.assertEqual(correction["changed_fields"], 3)
        self.assertEqual(correction["provenance_events_before"], 10370)
        self.assertEqual(correction["provenance_events_after"], 10373)
        self.assertEqual(correction["validator"], "PASS")
        self.assertEqual(correction["batch_artifact_sync"], "PASS")

    def test_batch12_correction03_qa_pass(self):
        correction = json.loads(CORRECTION03_QA_OUT.read_text(encoding="utf-8"))
        self.assertEqual(correction["result"], "PASS")
        self.assertEqual(correction["correction_id"], "MADORAN-ENRICH-012-CORRECTION-03")
        self.assertEqual(correction["corrected_rows"], ["709", "714", "715", "721", "731", "734", "736", "739", "745", "746", "749", "752", "753", "756", "759"])
        self.assertEqual(correction["changed_fields"], 19)
        self.assertEqual(correction["provenance_events_before"], 10373)
        self.assertEqual(correction["provenance_events_after"], 10392)
        self.assertEqual(correction["validator"], "PASS")
        self.assertEqual(correction["batch_artifact_sync"], "PASS")

    def test_batch12_correction04_qa_pass(self):
        correction = json.loads(CORRECTION04_QA_OUT.read_text(encoding="utf-8"))
        self.assertEqual(correction["result"], "PASS")
        self.assertEqual(correction["correction_id"], "MADORAN-ENRICH-012-CORRECTION-04")
        self.assertEqual(correction["corrected_rows"], ["749"])
        self.assertEqual(correction["changed_fields"], 2)
        self.assertEqual(correction["provenance_events_before"], 10392)
        self.assertEqual(correction["provenance_events_after"], 10394)
        self.assertEqual(correction["validator"], "PASS")
        self.assertEqual(correction["batch_artifact_sync"], "PASS")

    def test_batch12_correction05_qa_pass(self):
        correction = json.loads(CORRECTION05_QA_OUT.read_text(encoding="utf-8"))
        self.assertEqual(correction["result"], "PASS")
        self.assertEqual(correction["correction_id"], "MADORAN-ENRICH-012-CORRECTION-05")
        self.assertEqual(correction["corrected_rows"], ["752", "759"])
        self.assertEqual(correction["changed_fields"], 4)
        self.assertEqual(correction["provenance_events_before"], 10394)
        self.assertEqual(correction["provenance_events_after"], 10398)
        self.assertEqual(correction["validator"], "PASS")
        self.assertEqual(correction["batch_artifact_sync"], "PASS")

    def test_batch12_correction06_qa_pass(self):
        correction = json.loads(CORRECTION06_QA_OUT.read_text(encoding="utf-8"))
        self.assertEqual(correction["result"], "PASS")
        self.assertEqual(correction["correction_id"], "MADORAN-ENRICH-012-CORRECTION-06")
        self.assertEqual(correction["corrected_rows"], ["705", "730", "734", "748", "752", "755", "759", "764", "768"])
        self.assertEqual(correction["changed_fields"], 17)
        self.assertEqual(correction["provenance_events_before"], 10398)
        self.assertEqual(correction["provenance_events_after"], 10415)
        self.assertEqual(correction["validator"], "PASS")
        self.assertEqual(correction["batch_artifact_sync"], "PASS")

    def test_full_validator_passes_after_batch12(self):
